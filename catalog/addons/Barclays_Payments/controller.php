...
class Barclays_Payments extends lC_Addon {
  /*
  * Class constructor
  */
  public function Barclays_Payments() {    
    global $lC_Language;    
   /**
  ...
  */
  public function install() {
    ...
    $lC_Database->simpleQuery("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('PSPID', 'ADDONS_PAYMENT_" . strtoupper($this->_code) . "_PSPID', '', 'The Account ID used for the Global Iris payment service.', '6', '0', now())");
    $lC_Database->simpleQuery("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Password', 'ADDONS_PAYMENT_" . strtoupper($this->_code) . "_PASSWORD', '', 'The API Token used for the Global Iris payment service.', '6', '0', now())");
  ...
  public function getKeys() {
    if (!isset($this->_keys)) {
      $this->_keys = array('ADDONS_PAYMENT_' . strtoupper($this->_code) . '_STATUS',
                           'ADDONS_PAYMENT_' . strtoupper($this->_code) . '_PSPID',
